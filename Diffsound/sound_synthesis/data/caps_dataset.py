...
class CapsDataset(Dataset):
    def __init__(self, data_root, phase = 'train', mel_num=80,
        ...
        if phase=='train':
            ...
            tmp_phase='val'
        self.image_folder = os.path.join(data_root, tmp_phase)
        self.root = os.path.join(data_root, phase)
        name_path = os.path.join(self.root, "name_list.pkl")
        self.name_list = pickle.load(open(name_path, 'rb'), encoding="bytes")
    ...
    def __getitem__(self, index):
        ...
        # caption 여러개 중에 랜덤으로 하나 고르는 부분 안 필요함
